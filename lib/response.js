...
class RESPONSE {
  ...
  header(key, value, append) {
    let _key = this.app._lowercaseHeaders ? key.toLowerCase() : key;
    ...
    if (!_key)
      return asArr
        ...
        : Object.keys(this._headers).reduce((headers, childKkey) => {
            let _childKey = this.app._lowercaseHeaders
              ? childKkey.toLowerCase()
              : childKkey;
  ...
  hasHeader(key) {
    let _key =
      this.app._lowercaseHeaders && key ? key.toLowerCase() : key || '';
