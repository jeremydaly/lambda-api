...
class RESPONSE {
  ...
  header(key,...values) {
    ...
    let _value = values === undefined ? '' : values instanceof Array ? values.length > 1 ? values : values[0] === undefined ? '' : values[0] : values
    this._headers[_key] = _value // set
    return this
  }
  ...
  send(body) {
    ...
    let headers = Object.keys(this._headers)
      .filter(key => !(this._headers[key] instanceof Array))
      .reduce((acc,e) => Object.assign(acc, {[e]: this._headers[e]}), {})

    let multiValueHeaders = Object.keys(this._headers)
      .filter(key => this._headers[key] instanceof Array)
      .reduce((acc,e) => Object.assign(acc, {[e]: this._headers[e]}), {})
    ...
    if (Object.keys(multiValueHeaders).length > 0) {
      Object.assign(this._response, {multiValueHeaders})
    }
