...
class RESPONSE {
  ...
  header(key,value,append) {
    let _key = this.app._headerStyle !== 'http2' ? key : key.toLowerCase(); // store as lowercase    
  ...
  getHeader(key,asArr) {
    let _key = this.app._headerStyle !== 'http2' ? key : (key || '').toLowerCase(); // store as lowercase    
    if (!_key) return asArr ? this._headers :
      Object.keys(this._headers).reduce((headers,childKkey) => {
        let _childKey = this.app._headerStyle !== 'http2' ? childKey : (childKey || '').toLowerCase(); // store as lowercase    
        Object.assign(headers, { [_childKey]: this._headers[_childKey].toString() })
        ,{}) // return all headers
    
    return asArr ? this._headers[key.toLowerCase()]
      : this._headers[_key] ?
        this._headers[_key].toString() : undefined
  }
