...
class REQUEST {
  ...
  async parseRequest() {
    ...
    // Set the raw headers
    this.rawHeaders = Object.assign({}, this.app._event.headers, this.app._event.multiValueHeaders)

    // Set the headers to lowercase
    this.headers = Object.keys(this.rawHeaders)
      .reduce((acc,header) => {
        const headerKey = header.toLowerCase()
        const headerValue = (this.rawHeaders[header] instanceof Array && this.rawHeaders[header].length === 1) ? this.rawHeaders[header][0] : this.rawHeaders[header]
        return Object.assign(acc, { [headerKey]: headerValue })
      }, {})
