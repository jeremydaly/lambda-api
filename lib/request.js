...
class REQUEST {
  ...
  // Gets a header field (case-insensitive)
  getHeader(userKey,asArr) {
    let key = (userKey || '').toLowerCase()

    return asArr ? this.headers[key]
      : this.headers[key] ?
        this.headers[key].toString() : undefined
  }
  ...
  getQuery(key,asArr) {
    return asArr ? this.multiValueQuery[key] : this.query[key]
  }
