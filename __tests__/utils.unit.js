...
describe('Utility Function Tests:', function() {
  ...
  describe('encodeUrl:', function() {
    ...
    it('Unencoded with space in param', function() {
      expect(utils.encodeUrl('http://www.github.com/?foo=bar with space')).toBe('http://www.github.com/?foo=bar%20with%20space')
    }) // end it

    it('Encoded URL with additional invalid sequence', function() {
      expect(utils.encodeUrl('http://www.github.com/?foo=bar%20with%20space%foo')).toBe('http://www.github.com/?foo=bar%20with%20space%25foo')
    }) // end it
    ...
    it('Encode special characters, double encode, decode', function() {
      ...
      expect(encoded).toBe('http://www.github.com/?foo=%D1%88%D0%B5%D0%BB%D0%BB%D1%8B')
      expect(doubleEncoded).toBe('http://www.github.com/?foo=%D1%88%D0%B5%D0%BB%D0%BB%D1%8B')
  ...
  describe('parseAuth:', function() {
    ...
    it('Basic', function() {
      ...
      expect(result).toEqual({ type: 'Basic', value: creds, username: 'test', password: 'testing' })
    }) // end it

    it('Basic (no password)', function() {
      let creds = Buffer.from('test').toString('base64')
      let result = utils.parseAuth('Basic ' + creds)
      expect(result).toEqual({ type: 'Basic', value: creds, username: 'test', password: null })
    }) // end it
  ...
  describe('mimeLookup:', function() {
    ...
    it('docx', function() {
      expect(utils.mimeLookup('docx')).toBe('application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    }) // end it

    it('Custom', function() {
      expect(utils.mimeLookup('.mpeg', { mpeg: 'video/mpeg' })).toBe('video/mpeg')
    }) // end it
  ...
  describe('generateEtag:', function() {
    ...
    it('Sample text', function() {
      expect(utils.generateEtag('this is a test string')).toBe('f6774519d1c7a3389ef327e9c04766b9')
    }) // end it

    it('Sample object', function() {
      expect(utils.generateEtag({ test: true, foo: 'bar' })).toBe('def7648849c1e7f30c9a9c0ac79e4e52')
    }) // end it

    it('Sample JSON string object', function() {
      expect(utils.generateEtag(JSON.stringify({ test: true, foo: 'bar' }))).toBe('def7648849c1e7f30c9a9c0ac79e4e52')
    }) // end it

    it('Sample buffer', function() {
      expect(utils.generateEtag(Buffer.from('this is a test string as a buffer'))).toBe('6a2f7473a72cfebc96ae8cf93d643b70')
    }) // end it

    it('Long string', function() {
      let longString = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.'
      expect(utils.generateEtag(longString)).toBe('2d8c2f6d978ca21712b5f6de36c9d31f')
    }) // end it

    it('Long string (minor variant)', function() {
      let longString = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est Laborum.'
      expect(utils.generateEtag(longString)).toBe('bc82a4065a8ab48ade900c6466b19ccd')
    }) // end it
  ...
  describe('parseS3:', function() {
    it('Valid S3 path', function() {
      expect(utils.parseS3('s3://test-bucket/key')).toEqual({ Bucket: 'test-bucket', Key: 'key' })
    })

    it('Valid S3 path (nested key)', function() {
      expect(utils.parseS3('s3://test-bucket/key/path/file.txt')).toEqual({ Bucket: 'test-bucket', Key: 'key/path/file.txt' })
    })
  ...
  describe('mergeObjects:', function() {
    ...
    it('Multiple Items', function() {
      ...
      expect(utils.mergeObjects(obj1,obj2)).toEqual({ 1: ['test','test2'], 2: ['testA','testB'] })
    })

    it('Missing Items (obj1)', function() {
      let obj1 = { 1: ['test'] }
      let obj2 = { 1: ['test2'], 2: ['testB'] }
      expect(utils.mergeObjects(obj1,obj2)).toEqual({ 1: ['test','test2'], 2: ['testB'] })
    })

    it('Missing Items (obj2)', function() {
      let obj1 = { 1: ['test'], 2: ['testA'] }
      let obj2 = { 1: ['test2'] }
      expect(utils.mergeObjects(obj1,obj2)).toEqual({ 1: ['test','test2'], 2: ['testA'] })
    })

    it('No similarities', function() {
      let obj1 = { 1: ['test'] }
      let obj2 = { 2: ['testA'] }
      expect(utils.mergeObjects(obj1,obj2)).toEqual({ 1: ['test'], 2: ['testA'] })
    })
  }) // end parseS3 tests
